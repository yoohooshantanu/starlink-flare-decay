...
def parse_flare_class(class_type: str) -> tuple[str, float]:
    ...
    For X-class flares the intensity is the sub-class × 10
    (so X1.1 → 11.0 on the M-scale for comparison purposes).
    """
...
def parse_source_location(loc: str | None) -> tuple[float | None, float | None]:
    ...
    Returns (latitude, longitude) in degrees.  East is positive.
    """
    if not loc:
        return (None, None)
...
def is_earth_directed(loc: str | None) -> bool:
    """True if source is within ±EARTH_DIRECTED_LON_LIMIT° of disk center."""
...
def fetch_flares() -> list[dict]:
    ...
    while cursor < end:
        ...
        chunk += 1
        print(f"  [DONKI] chunk {chunk}: {params['startDate']} → {params['endDate']} …", end=" ")
...
def main():
    ...
    # Summary
    m_count = sum(1 for f in flares if f["classLetter"] == "M")
    x_count = sum(1 for f in flares if f["classLetter"] == "X")
    ed_count = sum(1 for f in flares if f["earthDirected"])
    cme_count = sum(1 for f in flares if f["hasCME"])

    print()
    print(f"  Total M/X flares : {len(flares)}")
    print(f"    M-class        : {m_count}")
    print(f"    X-class        : {x_count}")
    print(f"    Earth-directed : {ed_count}")
    print(f"    With linked CME: {cme_count}")

    # Save
    config.FLARE_FILE.parent.mkdir(parents=True, exist_ok=True)
    with open(config.FLARE_FILE, "w") as f:
        json.dump(flares, f, indent=2)
    print(f"\n  ✓ Saved to {config.FLARE_FILE}")
